 ...
 *                   now includes introspection XML
 *
 */
...
const char *const INTROSPECT_XML = 
    "<node>"
    "  <interface name='in.softprayog.dbus_example'>"
    "    <method name='add_numbers'>"
    "      <arg type='s' name='numbers' direction='in'/>"
...
int main (int argc, char **argv)
{
    ...
    while (1) {
        ...
        if (dbus_message_is_method_call(message, "org.freedesktop.DBus.Introspectable", "Introspect")) {
            ...
            if (!dbus_message_iter_append_basic(&iter, DBUS_TYPE_STRING, &INTROSPECT_XML)) {
